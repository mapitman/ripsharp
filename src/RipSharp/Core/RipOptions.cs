...
public class RipOptions
{
    ...
    public ErrorMode ErrorMode { get; set; } = ErrorMode.Prompt;
    ...
    public static RipOptions ParseArgs(string[] args)
    {
        ...
        for (int i = 0; i < args.Length; i++)
        {
            ...
            switch (a)
            {
                ...
                case "--concurrency":
                    if (int.TryParse(next(), out var c))
                    {
                        if (c < 1 || c > 8)
                            throw new ArgumentException("--concurrency must be between 1 and 8");
                        opts.Concurrency = c;
                    }
                    break;
                case "--error":
                    var errorMode = next()?.ToLowerInvariant();
                    opts.ErrorMode = errorMode switch
                    {
                        "ignore" => ErrorMode.Ignore,
                        "prompt" => ErrorMode.Prompt,
                        _ => throw new ArgumentException("--error must be 'prompt' or 'ignore'")
                    };
                    break;
            }
        }
    ...
    public static void DisplayHelp(IConsoleWriter writer)
    {
        ...
        OptionLine("--season N", "Season number (TV only)");
        OptionDetail("Default: auto-detect from output path, or 1");
        OptionLine("--episode-start N", "Starting episode number (TV only)");
        OptionDetail("Default: auto-detect from existing files, or 1");
        ...
        OptionLine("--concurrency N", "Number of concurrent encodes (1-8, default: 1)");
        OptionLine("--preview", "Show rip plan and confirm before starting");
        OptionLine("--error prompt|ignore", "Error handling mode (default: prompt)");
        OptionDetail("- prompt: Ask to retry/continue/abort on errors");
        OptionDetail("- ignore: Log errors and continue");
